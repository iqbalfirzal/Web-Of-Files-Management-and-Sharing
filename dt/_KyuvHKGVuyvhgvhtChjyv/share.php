<?php
	...
	$usrshr = getCurrentDirectory();
	$usrshrfix = str_replace('_', ' ', $usrshr);
	$title = "Foldernya $usrshrfix";
	include "../../int/db.php";
    $query = mysql_query("SELECT status FROM shareall WHERE user='$usrshrfix'");
    while ($row = mysql_fetch_assoc($query)) {
        $cekiki = $row['status'];
        if ($cekiki == "0") 
		{
			session_start();
			if(!$_SESSION['telahmasuk'] OR $_SESSION['pengguna']!==$usrshrfix)
			{
				header("location:../../directory_locked.php?of=$usrshr");
			}
		}
	}
	$ignore_file_list = array( ".htaccess", "Thumbs.db", ".DS_Store", "index.php", "_YOUR_FILE_PLACED_HERE_", "share.php" );
	$ignore_ext_list = array( "php","html" );
...
function display_block( $file )
{
	...
	$rtn = "<div class=\"block\">";
	$rtn .= "<a href=\"../../!?o=$loc&f=$file\" class=\"$file_ext\"{$download_att}>";
	$rtn .= "	<div class=\"img $file_ext\">&nbsp;</div>";
	$rtn .= "	<div class=\"name\">\n";
	$rtn .= "		<div class=\"file\">" . basename($file) . "</div>\n";
	$rtn .= "		<div class=\"date\">Ukuran: " . format_size($file) . "<br />Diunggah: " .  date("D. F jS, Y - h:ia", filemtime($file)) . "</div>\n";
	$rtn .= "	</div>\n";
	$rtn .= "	</a>\n";
	$rtn .= "</div>";
	return $rtn;
...
<div style="padding: 10px 10px 40px 10px;"><a href="http://bagiin.ga">Free Data File Host</a> | <a href="http://megonodev.ga/tel">Official Website</a></div>
</body>
</html>